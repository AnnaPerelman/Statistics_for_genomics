...
author: "Jean-Philippe Fortin, Leslie Myint"
date: "April 3, 2014"
...
- become more familiar with Affymetrix microarray data
- produce exploratory and diagnostic plots (boxplots, densities, MA plots)
...
First load the **affy** and **affydata** packages from Bioconductor: 
...
for (i in 2:4) {
	lines(density(log.raw[,i]), col = i, lwd = 3)
}
```
...
```{r}
plot(log.raw[,1], log.raw[,2], pch = 16, cex = 0.5)
abline(a = 0, b = 1, col = "deepskyblue", lwd = 4)
```
...
lines(lowess.curve, col = "deeppink", lwd = 4)
```
...
for (i in 2:4) {
	lines(density(qn[,i]), col = i, lwd = 3)
}
```
...
The lowess curve overlaps the $y = 0$ line.

## Exercise 1

- Read about lowess normalization. 
...
lines(lowess.curve, col = "deeppink", lwd = 3)
...
```{r}
library(scales)
plot(avg, diff, pch = 16, cex = 0.5, col = alpha("black", 0.1))
lines(lowess.curve, col = "deeppink", lwd = 3)
```

## Probe sets

A set of probes targeting the same gene is called a **probeset** (http://www.affymetrix.com/support/help/faqs/mouse_430/faq_8.jsp). First, let's get the probe name for each row of the PM (or MM) matrix:
...
The first 16 rows of the PM matrix are probes that belong to the same probeset ("100_g_at").
...
```{r}
p <- fit2$p.value[,2]
plot(fit2$coefficients[,2], -log2(p), pch=16, cex=0.3)
```
